...
import com.zhenl.launcher.get
import com.zhenl.launcher.launcherPrefs
import com.zhenl.launcher.toTitleCase
...
class IconPackImpl(context: Context, packPackageName: String) : IconPack(context, packPackageName) {
    ...
    private val prefs by lazy { context.launcherPrefs }
    ...
    override fun loadPack() {
        try {
            ...
            Log.d(TAG, "completed parsing pack $packPackageName in ${endTime - startTime}ms")
            return
    ...
    override fun getIcon(entry: IconPackManager.CustomIconEntry, iconDpi: Int): Drawable? {
        ...
        if (drawableId != 0) {
            try {
                ...
                Log.e(TAG, "Can't get drawable for name ${entry.icon} ($drawableId)", ex)
            }
        }
        return null
    }
    ...
    override fun getIcon(launcherActivityInfo: LauncherActivityInfo, iconDpi: Int,
        ...
        if (drawableId != 0) {
            try {
                ...
                Log.e(TAG, "Can't get drawable for $component ($drawableId)", ex)
            }
        }
    ...
    override fun getAllIcons(callback: (List<PackEntry>) -> Unit, cancel: () -> Boolean, filter: (item: String) -> Boolean) {
        ...
        try {
            ...
            Log.d(TAG, "initialized parser for pack $packPackageName in ${System.currentTimeMillis() - startTime}ms")
    ...
    fun getDrawable(name: String, density: Int): Drawable? {
        ...
        return try {
            ...
            Log.e(TAG, "Can't get drawable $id($name) from $packPackageName", ex)
            null
        }
    }
